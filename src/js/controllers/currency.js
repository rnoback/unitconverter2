(function () {
    ...
    converterApp.controller('currencyController', 
        ['$scope', '$filter', '$location', '$resource', 'unitSelectionService', 'currencyValueFactory',
        function($scope, $filter, $location, $resource, unitSelectionService, currencyValueFactory) {
   ...
   // $scope.currencyValues = $resource('http://api.fixer.io/latest?base=EUR', {callback: "JSON_CALLBACK"}, {get:{method: "JSONP" }}  );
    ...
    function parseData(data){
        ...
        $scope.$watch('unit26.value', function(){
            $scope.unit1.value = formatFloat($scope.unit26.value / $scope.unit26.factor);
        });
        $scope.$watch('unit27.value', function(){
            $scope.unit1.value = formatFloat($scope.unit27.value / $scope.unit27.factor);
        });
        $scope.$watch('unit28.value', function(){
            $scope.unit1.value = formatFloat($scope.unit28.value / $scope.unit28.factor);
        });
        $scope.$watch('unit29.value', function(){
            $scope.unit1.value = formatFloat($scope.unit29.value / $scope.unit29.factor);
        });
        $scope.$watch('unit30.value', function(){
            $scope.unit1.value = formatFloat($scope.unit30.value / $scope.unit30.factor);
        });
        $scope.$watch('unit31.value', function(){
            $scope.unit1.value = formatFloat($scope.unit31.value / $scope.unit31.factor);
        });
        $scope.$watch('unit32.value', function(){
            $scope.unit1.value = formatFloat($scope.unit32.value / $scope.unit32.factor);
        });
        ...
        $scope.$watch('unit1.value', function(){
            ...
            $scope.unit26.value = formatFloat($scope.unit1.value * $scope.unit26.factor);
            $scope.unit27.value = formatFloat($scope.unit1.value * $scope.unit27.factor);
            $scope.unit28.value = formatFloat($scope.unit1.value * $scope.unit28.factor);
            $scope.unit29.value = formatFloat($scope.unit1.value * $scope.unit29.factor);
            $scope.unit30.value = formatFloat($scope.unit1.value * $scope.unit30.factor);
            $scope.unit31.value = formatFloat($scope.unit1.value * $scope.unit31.factor);
            $scope.unit32.value = formatFloat($scope.unit1.value * $scope.unit32.factor);
        });
